...
def entry():
    try:
        subprocess.run(["python", "garage1_enter.py"])  
    except FileNotFoundError:
        print("Error: garage1_enter.py not found.")

# labels
welcome_label = tk.Label(text="Welcome to Stack Garage!", font=("Helvetica", 24, "bold"), background="#A2CFFE", fg="#003366")
welcome_label.place(relx=0.3, rely=0.2)
welcome_label.configure()

label1 = tk.Label(text="What can I do for you?", font="Helvetica, 15", background="#A2CFFE", fg="#003366")
label1.place(relx=0.405, rely=0.28)
...
def leave():
    try:
        subprocess.run(["python", "garage1_exit.py"])  
    except FileNotFoundError:
        print("Error: garage1_exit.py not found.")
...
def records_stack():
    try:
        subprocess.run(["python", "records_stack.py"])  
    except FileNotFoundError:
        print("Error: records_stack.py not found.")

records_button = tk.Button(window, text="Know Records", font=("Helvetica", 16, "bold"), command=records_stack)
